...
def send_welcome_notification(user, password=None):
    ...
    if user.role == 'patient':
        message = (
            ...
            f"Welcome to the MediCare Patient Portal! Your account has been successfully created.\n\n"
            f"You can now log in to book appointments and view your medical history.\n"
            f"Username: {user.username}\n\n"
            f"Regards,\nMediCare Management"
        )
        sms_message = f"Dear {user.get_full_name() or user.username},\nWelcome to MediCare! Your patient account ({user.username}) is ready. Log in to book appointments."
    else:
        ...
        sms_message = f"Dear {user.get_full_name() or user.username},\n\nAccount created for you at MediCare HMS as {role_display}."
...
def send_appointment_notification(appointment):
    ...
    # Notify Patient
    subject_p = f"Appointment Confirmation – {appointment.appointment_date}"
    message_p = (
    ...
    sms_p = f"Dear {patient.full_name},\n\nAppointment confirmed for {appointment.appointment_date} at {appointment.appointment_time.strftime('%I:%M %p')} with Dr. {doctor_name}. Status: {appointment.get_status_display()}."
    ...
    if doctor:
        ...
        if doctor.phone:
            sms_d = f"Dear Dr. {doctor.last_name},\n\nNew appointment with {patient.full_name} on {appointment.appointment_date} at {appointment.appointment_time.strftime('%I:%M %p')}. Status: {appointment.get_status_display()}."
...
def send_appointment_update_notification(appointment):
    ...
    # Notify Patient
    subject = f"Appointment Updated – {appointment.appointment_date}"
    message = (
        f"Dear {patient.full_name},\n"
        f"Your appointment details have been updated.\n\n"
    ...
    sms_p = f"Dear {patient.full_name},\n\nYour appointment on {appointment.appointment_date} has been updated to {appointment.appointment_time.strftime('%I:%M %p')}. Status: {appointment.get_status_display()}."
    send_sms_notification(patient.phone, sms_p)
    ...
    if doctor:
        ...
        send_email_notification(doctor.email, subject_d, message_d)
        if doctor.phone:
            sms_d = f"Dear Dr. {doctor.last_name},\n\nAppointment with {patient.full_name} on {appointment.appointment_date} updated to {appointment.appointment_time.strftime('%I:%M %p')}. Status: {appointment.get_status_display()}."
            send_sms_notification(doctor.phone, sms_d)
...
def send_appointment_cancellation_notification(appointment):
    ...
    message = (
        f"Dear {patient.full_name},\n"
        f"Your appointment scheduled for {appointment.appointment_date} has been cancelled.\n\n"
    ...
    if doctor:
        ...
        send_email_notification(doctor.email, subject_d, message_d)
        if doctor.phone:
            sms_d = f"Dear Dr. {doctor.last_name},\n\nYour appointment with {patient.full_name} on {appointment.appointment_date} has been cancelled."
            send_sms_notification(doctor.phone, sms_d)
...
def send_appointment_reminder(appointment):
    ...
    subject = f"Reminder: Appointment Tomorrow – {appointment.appointment_date}"
    message = (
        f"Dear {patient.full_name},\n"
    ...
    send_email_notification(patient.email, subject, message)
    sms_msg = f"Dear {patient.full_name},\n\nReminder: You have an appointment tomorrow at {appointment.appointment_time.strftime('%I:%M %p')}. Status: {appointment.get_status_display()}."
    send_sms_notification(patient.phone, sms_msg)
    
    if doctor and doctor.phone:
        sms_d = f"Dear Dr. {doctor.last_name},\n\nReminder: You have an appointment tomorrow with {patient.full_name} at {appointment.appointment_time.strftime('%I:%M %p')}. Status: {appointment.get_status_display()}."
        send_sms_notification(doctor.phone, sms_d)
